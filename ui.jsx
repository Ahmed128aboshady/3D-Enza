// ============ Enza Trade — shared UI components ============
const { useState, useEffect, useRef, useContext } = React;

// store hook — context is created in app.jsx
const useStore = () => useContext(window.ENZA_CTX);
...
const Logo = ({ variant }) => {
  ...
  return (
    <button className="brand" onClick={() => navigate('home')} aria-label="Enza Trade home">
      <img src="logo.png?v=2" alt="" style={{ height: 40 }} />
...
const ProductCard = ({ p }) => {
  ...
  return (
    <article className="card" onClick={() => navigate('product', { id: p.id, vi })}>
        ...
        {anyStock && (
          <div className="card-quick" onClick={(e) => e.stopPropagation()}>
            <button className="btn btn-primary btn-block" onClick={() => { addToCart(p, p.variants.find(x => x.stock), 1); toast(t({ en: 'Added to cart', ar: 'اتضاف للسلة' })); }}>
              <Icon n="plus" className="quick-add-icon" style={{ display: 'none', width: 16, height: 16 }} />
              <span className="quick-add-text">{t({ en: 'Quick add', ar: 'أضف بسرعة' })}</span>
            </button>
          </div>
        )}
      </div>
        ...
        <div className="row between">
          ...
          {p.variants.length > 1 && (
            <div className="card-swatches" onClick={(e) => e.stopPropagation()}>
              {p.variants.slice(0, 5).map((x, i) => (
                <button
                  key={i}
                  className={'sw ' + (i === vi ? 'active' : '')}
              ...
              {p.variants.length > 5 && (
                <span className="sw-more">+{p.variants.length - 5}</span>
              )}
            </div>
          )}
        </div>
...
const Footer = () => {
  ...
  // Shop column: "All Materials" first, then the live categories. Wrap to 2 cols if many.
  const shopCats = (categories || CATEGORIES).filter(c => c.id !== 'all');
  const shopLinks = [{ id: 'all', label: { en: 'All Materials', ar: 'كل الخامات' } }, ...shopCats.map(c => ({ id: c.id, label: c }))];
  const socials = [
  ...
  return (
    <footer className="ftr">
      ...
      <div className="ftr-bottom">
        <span>© {new Date().getFullYear()} Enza Trade · {t(fc.founded || { en: 'Founded in Egypt', ar: 'صُنع في مصر' })}</span>
        {socials.length > 0 && (
