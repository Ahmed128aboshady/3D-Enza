// ============ Enza Trade — app root, state, routing ============
const { useState: aUS, useEffect: aUE, useMemo: aUM, useCallback } = React;

window.ENZA_CTX = React.createContext(null);

const LS = {
  get: (k, d) => { try { const v = localStorage.getItem('enza_' + k); return v == null ? d : JSON.parse(v); } catch { return d; } },
  set: (k, v) => { try { localStorage.setItem('enza_' + k, JSON.stringify(v)); } catch {} },
};

// default owner login — change from the Admin panel (Settings)
const DEFAULT_CREDS = { user: 'enza', pass: 'enza2026' };
...
function App() {
  ...
  const [content, setContent] = aUS(() => {
    const cached = LS.get('content', {});
    if (!cached.hero || !cached.about) {
      return SITE_CONTENT;
    }
    // Force update social links to the new values
    if (cached.footer && cached.footer.social) {
      cached.footer.social.facebook = SITE_CONTENT.footer.social.facebook;
      cached.footer.social.whatsapp = SITE_CONTENT.footer.social.whatsapp;
    }
    return { ...SITE_CONTENT, ...cached };
  });
  ...
  const navigate = useCallback((name, params = {}) => {
    ...
    if (name === 'product' && params.id) {
      h = '#/product/' + params.id + (params.vi != null ? '/' + params.vi : '');
    }
  ...
  const placeOrder = useCallback((f, shipping) => {
    ...
    LS.set('orderSeq', seq);
    const id = 'EZ' + String(seq).padStart(4, '0');
  ...
  return (
    <window.ENZA_CTX.Provider value={store}>
      <div className="app">
        <Header />
        <main><Page /></main>
        <Footer />
      ...
      <TweaksPanel title="Tweaks">
        ...
        <TweakColor label={lang === 'ar' ? 'لون الأكسنت' : 'Accent'} value={tw.accent}
          options={['#ed1e26', '#f11f2a', '#7a3b1a', '#2b2422']}
